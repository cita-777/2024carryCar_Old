...
输入参数：      要发送的字符串
返回值：        无
修改记录：tjc_send_val("n0", "val", 100); 发出的数据就是 n0.val=100
**********************************************************/
...
日期：      2024.09.18
功能：      串口发送浮点数值和结束符
输入参数：      要发送的对象名、属性和浮点数值
返回值：        无
示例:           tjc_send_float("t0", "val", 123.45); 发出的数据就是t0.val=123.45
修改记录：
**********************************************************/
void tjc_send_float(char* objname, char* attribute, float value)
{
    char buffer[32]=""; // 用于存储格式化后的浮点数值字符串
...
修改记录：
**********************************************************/
void TJC_HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
